...
#Split the data (D,classes)
Data = dataset.drop('class', axis=1)  
classes = dataset['class']

#Split the data and do resampling
Data_train, Data_test, classes_train, classes_test = train_test_split(Data,classes, test_size = 0.10)  

#Apply SVC classifier using a linear model
classifier = SVC(kernel='linear')  
#Fit the data using the train data
